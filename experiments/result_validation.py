...
from src.data import stratified_folds, stratified_validation_split
...
def load_group(current_settings, dataset, partitions):
    ...
    for fold_id in range(settings["num_folds"]):
        ...
        result_path = get_result_path(expected_settings, result_type)

        with open(result_path, encoding="utf-8") as result_file:
            result = json.load(result_file)

        if result["purpose"] != result_type:
            raise ValueError(f"Incorrect result purpose: {result_path}")

        if result["settings"] != expected_settings:
            raise ValueError(f"Incorrect result settings: {result_path}")
        ...
        if result["dataset"] != {
            ...
            raise ValueError(f"Incorrect dataset settings: {result_path}")
        ...
        if result["feature_policy"] != {
            ...
            raise ValueError(f"Incorrect feature policy: {result_path}")
        ...
        if (
            ...
            raise ValueError(f"Incorrect checkpoint selection: {result_path}")
        ...
        if labels != expected_labels:
            raise ValueError(f"Incorrect outer-test labels: {result_path}")

        if len(predictions) != len(labels):
            raise ValueError(f"Misaligned predictions and labels: {result_path}")
        ...
        if not isinstance(source_commit, str) or not source_commit:
            raise ValueError(f"Invalid source commit: {result_path}")

        results.append(result)

    if len({result["source_commit"] for result in results}) != 1:
        raise ValueError("Outer folds in one result group have different source commits")

    expected_parameters = results[0]["parameters"]

    if any(result["parameters"] != expected_parameters for result in results):
        raise ValueError("Parameter counts differ across outer folds")
