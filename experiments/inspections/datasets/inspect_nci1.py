...
print(
    "Graph task: chemical-compound activity classification "
    "for non-small-cell lung cancer screening"
)
print("Nodes: categorically labelled atoms")
print("Edges: molecular connectivity")
print(
    "Graph targets: processed numeric class IDs; "
    "no active/inactive meaning is assigned to 0 or 1 here"
)
...
for graph_number, index in enumerate(
    ...
    if node_features is None:
        ...
        active_channels = (
            node_features.argmax(dim=1)
            .unique()
            .sort()
            .values
        )

        print(f"Active node-feature channels: {active_channels}")
...
print()
print("Categorical node-feature vocabulary")
...
if node_feature_tensors:
    ...
    distinct_node_features = all_node_features.unique(dim=0)

    binary_entries = torch.logical_or(
        all_node_features == 0,
        all_node_features == 1,
    ).all().item()

    one_active_entry = (
        all_node_features.sum(dim=1) == 1
    ).all().item()

    one_hot = binary_entries and one_active_entry

    print(
        f"Number of distinct loaded rows: "
        f"{distinct_node_features.shape[0]}"
    )

    print(f"All loaded rows are one-hot: {one_hot}")

    if one_hot:
        active_channels = (
            all_node_features.argmax(dim=1)
            .unique()
            .sort()
            .values
        )

        print(f"Active channel indices: {active_channels}")
    else:
        print("Distinct rows:")
        print(distinct_node_features)
else:
