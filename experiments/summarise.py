...
def print_resource_table(
    ...
    print(
        "Training passes only; time totals cover all outer folds."
    )

    print(
        f"Dataset | {label_name} | Total parameters | "
        "Trainable parameters | Total time (s) | "
        "Mean epoch time (s)"
    )
...
def get_paired_accuracy_differences(
    ...
    for baseline_result, comparison_result in zip(
        ...
        if comparison_result["settings"]["fold_id"] != fold_id:
            raise ValueError(
                f"Unmatched outer folds: {dataset_name}"
            )

        if (
            baseline_result["partitions"]
            != comparison_result["partitions"]
        ):
            raise ValueError(
                f"Unmatched partitions: "
                f"{dataset_name}, fold {fold_id}"
            )
...
def print_gat_gatv2_accuracy_comparison(
    ...
    print(
        "Differences are GATv2 minus GAT in percentage points."
    )
...
def print_gat_gatv2_parameter_comparison(
    ...
    for dataset_name in datasets:
        ...
        gatv2_parameters = (
            reference_gatv2_groups[dataset_name][0]["parameters"]
        )
...
def print_learned_uniform_accuracy_comparison(
    ...
    print("Outer-test accuracy comparison:")
    print(
        "Differences are learned GAT minus Uniform GAT "
...
def main():
    ...
    first_result = check_runtime_conditions(
        all_results
    )
    ...
    print(
        "PyTorch:",
        first_result["pytorch_version"],
    )
    print(
        "PyG:",
        first_result["pyg_version"],
    )
    print(
        "CUDA build:",
        first_result["cuda_version"],
    )

    print_source_commits(
        all_results
    )
    ...
    for dataset_name in datasets:
        ...
        partitions = dataset_information[
            dataset_name
        ]["partitions"]
        ...
        for variant in head_variants:
            current_settings = get_variant_settings(
                variant
            )
            current_settings["dataset"] = dataset_name
        ...
        expected_parameters = (
            dataset_head_groups[0][0]["parameters"]
        )
        ...
        if any(
            ...
            raise ValueError(
                f"Parameter counts differ across head counts: "
                f"{dataset_name}"
            )
    ...
    print()
    print(
        "RQ1 fixed-width GAT head count:"
    )
    print(
    ...
    for dataset_name in datasets:
        ...
        rq2_results.extend(
            reference_gatv2_groups[dataset_name]
        )
    ...
    print()
    print(
        "RQ2 matched GAT and GATv2:"
    )
    print(
        "Validated reused reference fits:",
        len(rq2_results),
    )
    print("New fits: 0")
    ...
    for dataset_name in datasets:
        ...
        learned_results = reference_gat_groups[
            dataset_name
        ]

        learned_parameters = learned_results[0][
            "parameters"
        ]

        uniform_parameters = uniform_results[0][
            "parameters"
        ]

        if (
            uniform_parameters["total"]
            != learned_parameters["total"]
        ):
            raise ValueError(
                f"Total parameters differ for uniform control: "
                f"{dataset_name}"
            )

        if (
            learned_parameters["trainable"]
            != learned_parameters["total"]
        ):
            raise ValueError(
                f"Reference GAT has frozen parameters: "
                f"{dataset_name}"
            )

        if not (
            0
            < uniform_parameters["trainable"]
            < uniform_parameters["total"]
        ):
            raise ValueError(
                f"Uniform GAT does not have frozen parameters: "
                f"{dataset_name}"
            )
    ...
    print()
    print(
        "RQ3 learned versus uniform-attention retraining:"
    )
    print(
        "Validated fits including reused reference:",
        len(rq3_results),
